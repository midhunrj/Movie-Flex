...
import { updateScreen, UpdateScreen } from '../../redux/theatre/theatreThunk';
 ...
 useEffect(()=>{
  ...
  const handleTierChange = (e) => {
    ...
    const updatedTiers = Array(tierCount).fill().map((_, i) => ({
      name: screenData.tierDatas[i]?.name || '',
      seats: screenData.tierDatas[i]?.seats || '',
      ticketRate: screenData.tierDatas[i]?.ticketRate || '',
    }));
    setScreenData({ ...screenData, tierDatas: updatedTiers });
  };

  const handleFieldTierChange = (index, field, value) => {
    const updatedTiers = [...screenData.tierDatas];
  ...
  const handleAddTier=()=>{
    setScreenData({
      ...screenData,
      tierDatas:[...screenData.tierDatas,{name:'',
        seats: '',
        ticketRate:''}]
    })
  }
  ...
  const handleAddShowtime = () => {
    ...
    setScreenData({
      ...screenData,
      showtimes: [...(screenData.showtimes || []), { time: formattedTime, movie: '' }],
    });
  ...
  const handleRemoveTier = (index) => {
    const updatedTiers = screenData.tierDatas.filter((_, i) => i !== index);
...
<Modal
  ...
  {/* <EnrollMovieModal
        ...
        movies={screenData.enrolledMovies}
            ...
            <div className='flex justify-between gap-6'>
              <div className='flex flex-1 items-center gap-4'>
                ...
                <input
                  type="number"
                  name="Tier"
                  value={screenData.tierDatas.length}
...
//               {screenData.tierDatas.map((tier, index) => (
