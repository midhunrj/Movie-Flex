...
import { TierConfig } from './tierSeats';
...
const TierConfigModal: React.FC<TierConfigModalProps> = ({ onSave, onCancel, initialConfig }) => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded-lg shadow-lg w-1/3">
        ...
        <div className="flex justify-end space-x-4">
          <button onClick={onCancel} className="px-4 py-2 min-h-8 bg-gray-300 rounded">Cancel</button>
          <button onClick={handleSave} className="px-4 py-2 min-h-8 bg-blue-500 text-white rounded">Save</button>
        </div>
      </div>
    </div>
  );
