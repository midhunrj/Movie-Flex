...
const MovieDetails = () => {
  ...
  const { upcomingMovies, nowShowingMovies, user } = useSelector(
    (state: RootState) => state.user
  );
  const movie =
    upcomingMovies.find((movie) => movie.id === id) ||
    nowShowingMovies.find((movie) => movie.id === id);
  ...
  const fetchFavourites = async () => {
    const response = await userAuthenticate.get<Favorite[]>("/favourites", {
      params: { userId },
    });
  ...
  const handleFavoriteToggle = async (movieId: string) => {
    ...
    if (!isFavorite) {
      ...
      await userAuthenticate.delete("/remove-favourite", {
        data: { movieId, userId },
      });
    }
  };
  ...
  const similarMovies = [...upcomingMovies, ...nowShowingMovies]
    .filter((m) =>
      m.genre.some((g) => movie.genre.includes(g) && m.id !== movie.id)
    )
    .slice(0, 6);
    ...
    <>
      ...
      <div className="movie-details-page relative bg-gray-100">
        <div className="absolute top-4 left-4 z-50">
          <button
            className="bg-gray-100 bg-opacity-80  min-h-8  h-fit text-opacity-100 text-slate-900 w-fit hover:text-gray-300 hover:bg-transparent p-2 rounded-md"
            onClick={() => navigate(-1)}
          >
            <FaAngleLeft size={30} />
          </button>
        ...
        <div className="relative ">
          ...
          {!isVideoPlaying && (
            <div className="absolute top-28 left-1/4 p-4 flex gap-2 flex-col text-white z-30">
              ...
              {!upcoming &&(<p className="text-lg flex p-2 w-fit  bg-gray-600 bg-opacity-50 space-x-6 text-center items-center">
                Rating:
                {
                  <FaStar
                    size={26}
                    className={`ml-1 ${
                      movie.rating ? "text-red-500" : "text-gray-500"
                    }`}
                  />
                }
                <span className="ml-2">{movie.rating}/10</span>
                <button
                  className="p-2 bg-white min-h-8 w-fit h-fit text-base  text-slate-950"
              ...
              <p className="space-x-4 text-base  text-slate-100">
                {" "}
                <span>{MovieTime()}</span> <span>{movie.genre.join(",")}</span>{" "}
        ...
        {!isVideoPlaying && (
          <>
           {!upcoming &&(<div className="absolute z-40 bg-slate-200 shadow-md flex mx-8 justify-start space-x-2">
              <button
                className={`mt-2 p-2 rounded-lg min-h-8 w-fit text-gray-100 ${
                  isFavorite
                    ? "bg-gradient-to-br from-indigo-600 to-red-600"
                    : "bg-red-600"
              ...
              >
                <FaHeart
                  size={10}
                  className={`mr-2 ${
                    isFavorite ? "text-red-600" : "text-slate-200"
                  }`}
                />
            ...
            <div className="mt-12 p-4">
              ...
              <div className="mt-6 flex justify-center">
                <button
                  className="bg-green-500 p-2 min-h-8 w-fit h-fit rounded hover:bg-green-600 transition duration-300 cursor-pointer"
                  onClick={() => setIsModelOpen(true)}
                >
                  Add Your Rating
                </button>
              ...
              <div className="absolute inset-0 z-30 h-20 flex justify-center right-1/4 top-[19rem]  pointer-events-auto ">
                {!upcoming ? (
                  <button
                    className="bg-[#FF3C38] min-h-12 text-xl p-2 shadow-lg  rounded-lg  text-gray-200   w-48"
                  ...
                  <button
  className={`min-h-12 text-xl px-6 py-3 shadow-lg rounded-lg text-gray-100 w-48 flex justify-center items-center gap-2 transition-all duration-300 ${
    isFavorite ? "bg-red-500 w-fit" : "bg-[#006BFF]"
  }`}
                    style={{ cursor: "pointer" }}
                    onClick={()=>handleFavoriteToggle(movie.id!)}
                  >
                  ...
                  {similarMovies.slice(0, 4).map((similarMovie) => (
                    ...
                    >
                      ...
                      <div className="p-2 cursor-pointer ">
                        <h3 className="text-lg font-bold overflow-visible">
                          {similarMovie.title}
                        </h3>
                        <div className="flex  items-end align-text-bottom justify-end">
                          <button
                            ...
                            onClick={() =>
                              handleMovieNavigation(similarMovie.id!)
                            }
                          >
                            View Details
                          </button>
