...
const userData:string[]|null=storedUserData?JSON.parse(storedUserData):null
const storedtheatreData=localStorage.getItem('theatredata')
const theatreData:string[]|null=storedtheatreData?JSON.parse(storedtheatreData):null
...
.addCase(blockUser.pending, (state) => {
  ...
  .addCase(approveTheatre.fulfilled, (state, action) => {
    state.theatreData = state.theatreData.map((theatre) =>
      theatre._id === action.payload.theatre._id ? { ...theatre, is_approved:"Approved" } : theatre
    );
